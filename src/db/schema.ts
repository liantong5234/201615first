import { boolean, integer, pgTable, real, text, timestamp, index } from "drizzle-orm/pg-core";
...
// Image to Image AI Generator tables
export const imageTask = pgTable("image_task", {
	...
	numOutputs: integer("num_outputs").notNull().default(1),
	...
	imageTaskUserIdIdx: index("image_task_user_id_idx").on(table.userId),
	imageTaskStatusIdx: index("image_task_status_idx").on(table.status),
	imageTaskCreatedAtIdx: index("image_task_created_at_idx").on(table.createdAt),
}));

export const imageTaskInput = pgTable("image_task_input", {
	id: text("id").primaryKey(),
	taskId: text("task_id").notNull().references(() => imageTask.id, { onDelete: "cascade" }),
	...
	imageTaskInputTaskIdIdx: index("image_task_input_task_id_idx").on(table.taskId),
}));

export const imageTaskOutput = pgTable("image_task_output", {
	id: text("id").primaryKey(),
	taskId: text("task_id").notNull().references(() => imageTask.id, { onDelete: "cascade" }),
	storageKey: text("storage_key").notNull(),
	width: integer("width"),
	...
	imageTaskOutputTaskIdIdx: index("image_task_output_task_id_idx").on(table.taskId),
}));
