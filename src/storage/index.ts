...
import type { GetFileResult, StorageConfig, StorageProvider, UploadFileResult } from './types';
 ...
 * @returns Promise with the file data and content type
 */
export const getFile = async (key: string): Promise<GetFileResult> => {
  const provider = getStorageProvider();
...
export const getPublicUrl = (key: string): string => {
  const { publicUrl, endpoint, bucketName } = defaultStorageConfig;
